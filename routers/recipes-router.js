    ...
    Recipes.getShoppingList(id)
           .then(shoppingList => {
               if(shoppingList) {
                   ...
                   res.status(404).json({ message: "Recipe with specified ID not found" })
               }
           })
           .catch(error => {
               console.log({ error })
               res.status(500).json({ message: "Error retrieving list of ingredients for recipe with specified ID" })
           })
})
